...
def test_get_dados(client):
    with app.app_context():
        db.session.add(DadosColetados(user="joao", hostname="testhostname123", origem_ip="1.2.3.4"))
        db.session.commit()
    ...
    assert any(d["user"] == "joao" and d["hostname"] == "testhostname123" for d in data)

def test_delete_dado(client):
    with app.app_context():
        dado = DadosColetados(user="temp", hostname="testehostname123", origem_ip="1.1.1.1")
        db.session.add(dado)
        db.session.commit()
        id = dado.id
